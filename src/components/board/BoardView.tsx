      ...
      {isMobile && (
        <div className="flex border-b border-border flex-shrink-0 overflow-x-auto scrollbar-none">
          {columns.map((col) => (
            ...
            >
              {STATUS_LABELS[col.status]} ({col.tasks.length})
            </button>
          ))}
        </div>
      )}

      <div className="flex-1 flex gap-4 p-4 overflow-auto max-md:flex-col max-md:gap-2 max-md:p-3">
        {columns
          .filter((col) => !isMobile || col.status === mobileColumn)
          .map((col) => (
            <div
              key={col.status}
              className="flex-1 min-w-[200px] max-md:min-w-0 bg-muted/30 rounded-lg p-3"
